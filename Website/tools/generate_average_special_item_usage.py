...
def precomputeJSON(experimentLabel):
    ...
    GAME_TIME_LENGTH = expected_minutes(60);

    data = {}
    for player in intermediary_data:
        totalValue = 0
        for event in player['events']:
            totalValue += event['specialValue']
        data[player['_id']] = totalValue / GAME_TIME_LENGTH;
...
writeToFile(data, args.out if args.out != None else '../static_files/data/average_special_item_usage.json')